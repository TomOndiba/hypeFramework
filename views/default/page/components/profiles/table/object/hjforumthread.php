...
echo "<tr id=\"elgg-object-{$entity->guid}\" class=\"table-row $flagged_class\">";
...
foreach ($headers as $key => $value) {	
	if (is_array($value)) {
		...
		foreach ($value as $header => $sortable) {
			...
			echo elgg_view("page/components/profiles/elements/object/hjforumthread/$header", $vars);
			echo '</div>';
		}
		echo '</td>';
	} else {
		echo "<td class=\"$cell_class_namespace$key\">";
		echo elgg_view("page/components/profiles/elements/object/hjforumthread/$key", $vars);
		echo '</td>';
	}
